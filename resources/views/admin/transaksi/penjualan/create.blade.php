                ...
                <table class="table table-bordered" id="item-table">
                    ...
                    <tbody>
                        <tr class="item-row">
                            ...
                            <td><span class="subtotal-display">0</span><input type="hidden" class="subtotal" value="0"></td>
...
// Update diskon saat pilih pelanggan
document.getElementById('pelanggan-select').addEventListener('change', function() {
    const selectedOption = this.selectedOptions[0];
    diskonPersen = parseFloat(selectedOption.dataset.diskon) || 0;
...
// Event: pilih produk → isi harga & stok
document.addEventListener('change', function(e) {
    if (e.target.classList.contains('produk-select')) {
        const option = e.target.selectedOptions[0];
        const row = e.target.closest('tr');
        if (option.value) {
            row.querySelector('.harga').value = option.dataset.harga;
            row.querySelector('.stok-text').textContent = option.dataset.stok;
        } else {
            row.querySelector('.harga').value = '';
            row.querySelector('.stok-text').textContent = '0';
        }
        hitungSubtotal(row);
        hitungTotal();
    }
...
// Event: ubah jumlah atau harga → hitung ulang
...
function hitungTotal() {
    ...
    document.getElementById('diskon-display').textContent = 'Rp ' + diskonAmount.toLocaleString('id-ID');
    document.getElementById('total-display').textContent = 'Rp ' + total.toLocaleString('id-ID');
    document.getElementById('total-value').value = total;
}

// Hitung ulang saat halaman pertama kali load (jika ada default item)
